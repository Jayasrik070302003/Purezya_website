...
const targetDir = path.join(__dirname, '../../frontend/public/Product images');
...
async function run() {
    console.log('🚀 Starting Purazya Catalog Import from:', sourceDir);
    ...
    try {
        ...
        for (const cat of categoriesData) {
            ...
            for (const prod of cat.products) {
                ...
                const ext = path.extname(prod.file);
                const safeName = prod.name.toLowerCase().replace(/[^a-z0-9]/g, '-') + ext;
                const destFilePath = path.join(targetDir, safeName);

                // Copy image
                if (fs.existsSync(srcFilePath)) {
                    fs.copyFileSync(srcFilePath, destFilePath);
                    console.log(`   🖼️ Copied: ${prod.file} -> ${safeName}`);
                } else {
                    console.warn(`   ⚠️ File not found at: ${srcFilePath}`);
                }

                const publicImageUrl = `/Product images/${safeName}`;
                ...
                const prodRes = await client.query(
                    ...
                    [prod.name, cat.name, prod.description, prod.price, 100, publicImageUrl]
                );
        ...
        console.log(`\n🎉 Successfully imported ${categoriesData.length} categories and ${totalProducts} products!`);
